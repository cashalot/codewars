...
func IsValidWalk(walk []rune) bool {
	...
	var n, s, e, w int // создаем переменные для обозначения пути

	if len(walk) != 10 { // проверка на общую продолжительность пути.
		return false
	} else {
		// проход по циклу, что бы выверить сколько в каком направлении
		// было произведенно шагов.
		...
		// Проверяем условие возврата в исходное положение
		// по координамам X(n,s) и Y(e,w).
		// Если кол-во шагов "туда" и "обратно" равны,
		// значит человек вернулся на исходную точку.
		if n == s && e == w {
			return true
		} else {
			return false
